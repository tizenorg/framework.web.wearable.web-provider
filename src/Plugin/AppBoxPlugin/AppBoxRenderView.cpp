...
std::string AppBoxRenderView::getStartUrl(UrlType type, std::string& defaultParams)
{
    ...
    switch (type) {
    case URL_TYPE_BOX:
        path = dynamicbox_service_dbox_script_path(m_boxId.c_str());
        break;
    case URL_TYPE_PD:
        path = dynamicbox_service_gbar_script_path(m_boxId.c_str());
        break;
    default:
...
void AppBoxRenderView::showIcon(char *iconString, int width, int height)
{
    ...
    provider_send_updated(m_boxId.c_str(),m_instanceId.c_str(),width, height);
