...
BufferInfoPtr BoxRenderBuffer::acquireBuffer()
{
    ...
    bool touchable = true;
    char* type = web_provider_livebox_get_box_type(m_boxId.c_str());
...
{
    provider_send_updated(m_boxId.c_str(),m_instanceId.c_str(),m_width, m_height);
    provider_send_extra_info(m_boxId.c_str(),m_instanceId.c_str(), 0, m_contentInfo.c_str(), NULL, NULL, NULL);
}
        ...
        void* data)
{
    ...
    char* boxType = web_provider_livebox_get_box_type(This->m_boxId.c_str());
    TouchType type;
    switch (evt->type) {
